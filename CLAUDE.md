...
- When logging error messages, avoid dangling pointers from chained temporaries. Store the message in a variable first:
  ```cpp
  // WRONG - dangling pointer bug
  ESP_LOGE(TAG, "Failed: %s", timer.error().message().c_str());

  // CORRECT - message stored in variable
  auto message = timer.error().message();
  ESP_LOGE(TAG, "Failed: %s", message.c_str());
  ```
