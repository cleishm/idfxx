...
|-----------|-------------|---------------|
| **Core Infrastructure** | | |
| [idfxx_core](https://github.com/cleishm/idfxx/tree/main/components/idfxx_core) | Core utilities: error handling, memory allocators, chrono | [API Docs](https://cleishm.github.io/idfxx/group__idfxx__core.html) |
...
extern "C" void app_main() {
    ...
    if (!spi_bus_res) {
        auto msg = spi_bus_res.error().message();
        ESP_LOGE(TAG, "Failed to initialize SPI bus: %s", msg.c_str());
        return;
    }
    ...
    if (!panel_io_res) {
        auto msg = panel_io_res.error().message();
        ESP_LOGE(TAG, "Failed to create panel I/O: %s", msg.c_str());
        return;
    }
    ...
    if (!display_res) {
        auto msg = display_res.error().message();
        ESP_LOGE(TAG, "Failed to initialize display: %s", msg.c_str());
        return;
    }
    ...
    if (auto res = display->try_display_on(true); !res) {
        ...
        ESP_LOGE(TAG, "Failed to turn on display: %s", msg.c_str());
        return;
    }
...
Using exceptions for cleaner code flow:
