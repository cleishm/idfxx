...
event_loop::~event_loop() {
    if (_handle != nullptr) {
        ...
        if (err != ESP_OK) {
            auto message = make_error_code(err).message();
            ESP_LOGE(TAG, "Failed to delete event loop: %s", message.c_str());
        }
    }
}
...
void event_loop::unique_listener_handle::reset() noexcept {
    if (_handle._instance != nullptr) {
        ...
        if (!result) {
            ...
            ESP_LOGE(TAG, "Failed to unregister event handler: %s", message.c_str());
        }
