...
if (!loop_result) {
    auto message = loop_result.error().message();
    ESP_LOGE(TAG, "Failed to create event loop: %s", message.c_str());
    return;
}
...
if (!handle_result) {
    auto message = handle_result.error().message();
    ESP_LOGE(TAG, "Failed to register listener: %s", message.c_str());
    return;
}
```
...
loop->listener_add(event_base<wifi_event_t>{WIFI_EVENT},
    [](event_base<wifi_event_t>, wifi_event_t id, void* data) {
        ESP_LOGI(TAG, "WiFi event: %d", static_cast<int>(id));
    });
```
