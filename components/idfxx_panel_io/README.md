...
📚 **[Full API Documentation](https://cleishm.github.io/idfxx/group__idfxx__panel__io.html)**
...
Or add `idfxx_panel_io` to the `REQUIRES` list in your component's `CMakeLists.txt`.
...
try {
    ...
    // panel_io can now be used to create display panels or touch controllers

} catch (const std::system_error& e) {
    idfxx::log::error("PanelIO", "Error: {}", e.what());
}
```
...
if (!spi_bus_result) {
    idfxx::log::error("PanelIO", "Failed to create SPI bus: {}", spi_bus_result.error().message());
    return;
}
...
if (!panel_io_result) {
    idfxx::log::error("PanelIO", "Failed to create panel I/O: {}", panel_io_result.error().message());
    return;
}
...
```cpp
idfxx::panel_io::spi_config io_config{
    .cs_gpio = idfxx::gpio_5,
    .dc_gpio = idfxx::gpio_2,
    .spi_mode = 0,
    .pclk_freq = 40_MHz,
...
idfxx::panel_io panel_io(spi_bus, std::move(io_config));
```

### With Display Panel

The `panel_io` is typically passed to display panel drivers.
...
auto spi_bus = /* ... */;
idfxx::panel_io panel_io(
    spi_bus, idfxx::panel_io::spi_config{ /* ... */ });
...
- For SPI, the `dc_gpio` (Data/Command) is required for most display controllers; for I2C,
