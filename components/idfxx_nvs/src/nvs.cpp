...
static std::error_code make_nvs_error_code(esp_err_t e) noexcept {
    return make_errc(e).transform([](nvs::errc ec) { return std::error_code{std::to_underlying(ec), nvs_category()}; }
...
static result<nvs_handle_t> open_nvs(std::string_view namespace_name, bool read_only) {
    ...
    if (namespace_name.empty() || namespace_name.length() > 15) {
        return std::unexpected(std::error_code{std::to_underlying(nvs::errc::invalid_name), nvs_category()});
    }
    ...
    if (err != ESP_OK) {
        ...
        return std::unexpected(make_nvs_error_code(err));
    }
    return handle;
}
...
result<void> nvs::try_commit() {
    if (_read_only) {
        ...
        return std::unexpected(errc::read_only);
    }
