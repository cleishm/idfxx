...
C++ wrapper for ILI9341 LCD panel driver.
...
- Dual error handling API (result-based and exception-based)
...
if (!spi_bus_result) {
    auto error_msg = spi_bus_result.error().message();
    ESP_LOGE("LCD", "Failed to create SPI bus: %s", error_msg.c_str());
    return;
}
...
if (!panel_io_result) {
    auto error_msg = panel_io_result.error().message();
    ESP_LOGE("LCD", "Failed to create panel I/O: %s", error_msg.c_str());
    return;
}
...
if (!panel_result) {
    auto error_msg = panel_result.error().message();
    ESP_LOGE("LCD", "Failed to create panel: %s", error_msg.c_str());
    return;
}
...
ESP_LOGI("LCD", "Display initialized successfully");
```
