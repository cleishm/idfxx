...
// Unit tests for idfxx spi master wrapper
...
TEST_CASE("master_bus::make with valid config succeeds", "[idfxx][spi][hw]") {
    bus_config cfg{};
...
TEST_CASE("master_bus::make with invalid GPIO fails", "[idfxx][spi][hw]") {
    bus_config cfg{};
...
TEST_CASE("master_bus host() returns correct host", "[idfxx][spi][hw]") {
    bus_config cfg{};
    cfg.mosi_io_num = gpio_11;
    cfg.sclk_io_num = gpio_12;
    cfg.max_transfer_sz = 4096;
...
#if SOC_SPI_PERIPH_NUM > 2
TEST_CASE("master_bus::make with SPI3 host succeeds", "[idfxx][spi][hw]") {
    bus_config cfg{};
    cfg.mosi_io_num = gpio_11;
    cfg.sclk_io_num = gpio_12;
    cfg.max_transfer_sz = 4096;
...
TEST_CASE("master_bus::make with DMA disabled succeeds", "[idfxx][spi][hw]") {
    bus_config cfg{};
    cfg.mosi_io_num = gpio_11;
    cfg.sclk_io_num = gpio_12;
...
#if CONFIG_IDF_TARGET_ESP32
TEST_CASE("master_bus::make with specific DMA channel succeeds", "[idfxx][spi][hw]") {
    bus_config cfg{};
    cfg.mosi_io_num = gpio_11;
    cfg.sclk_io_num = gpio_12;
    cfg.max_transfer_sz = 4096;
...
TEST_CASE("master_bus destructor frees bus", "[idfxx][spi][hw]") {
    bus_config cfg{};
    cfg.mosi_io_num = gpio_11;
    cfg.sclk_io_num = gpio_12;
    cfg.max_transfer_sz = 4096;
