...
// mode
static_assert(std::to_underlying(idfxx::wifi::mode::null) == WIFI_MODE_NULL);
static_assert(std::to_underlying(idfxx::wifi::mode::sta) == WIFI_MODE_STA);
static_assert(std::to_underlying(idfxx::wifi::mode::ap) == WIFI_MODE_AP);
static_assert(std::to_underlying(idfxx::wifi::mode::ap_sta) == WIFI_MODE_APSTA);

// interface
static_assert(std::to_underlying(idfxx::wifi::interface::sta) == WIFI_IF_STA);
static_assert(std::to_underlying(idfxx::wifi::interface::ap) == WIFI_IF_AP);
...
// second_channel
static_assert(std::to_underlying(idfxx::wifi::second_channel::none) == WIFI_SECOND_CHAN_NONE);
...
// sae_pwe_method
static_assert(std::to_underlying(idfxx::wifi::sae_pwe_method::unspecified) == WPA3_SAE_PWE_UNSPECIFIED);
...
static_assert(std::to_underlying(idfxx::wifi::sae_pwe_method::both) == WPA3_SAE_PWE_BOTH);
...
// ip_event IDs
static_assert(std::to_underlying(idfxx::wifi::ip_event_id::sta_got_ip) == IP_EVENT_STA_GOT_IP);
static_assert(std::to_underlying(idfxx::wifi::ip_event_id::sta_lost_ip) == IP_EVENT_STA_LOST_IP);
static_assert(std::to_underlying(idfxx::wifi::ip_event_id::ap_sta_ip_assigned) == IP_EVENT_AP_STAIPASSIGNED);
static_assert(std::to_underlying(idfxx::wifi::ip_event_id::got_ip6) == IP_EVENT_GOT_IP6);
...
// clang-format off
static_assert(requires(const void* p) { { wifi::connected_info::from_opaque(p) } -> std::same_as<wifi::connected_info>; },
    "connected_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::disconnected_info::from_opaque(p) } -> std::same_as<wifi::disconnected_info>; },
    "disconnected_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::got_ip_info::from_opaque(p) } -> std::same_as<wifi::got_ip_info>; },
    "got_ip_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::scan_done_info::from_opaque(p) } -> std::same_as<wifi::scan_done_info>; },
    "scan_done_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::authmode_change_info::from_opaque(p) } -> std::same_as<wifi::authmode_change_info>; },
    "authmode_change_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::ap_sta_connected_info::from_opaque(p) } -> std::same_as<wifi::ap_sta_connected_info>; },
    "ap_sta_connected_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::ap_sta_disconnected_info::from_opaque(p) } -> std::same_as<wifi::ap_sta_disconnected_info>; },
    "ap_sta_disconnected_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::ap_probe_req_info::from_opaque(p) } -> std::same_as<wifi::ap_probe_req_info>; },
    "ap_probe_req_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::bss_rssi_low_info::from_opaque(p) } -> std::same_as<wifi::bss_rssi_low_info>; },
    "bss_rssi_low_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::home_channel_change_info::from_opaque(p) } -> std::same_as<wifi::home_channel_change_info>; },
    "home_channel_change_info requires from_opaque — layout differs from ESP-IDF struct");
static_assert(requires(const void* p) { { wifi::ftm_report_info::from_opaque(p) } -> std::same_as<wifi::ftm_report_info>; },
    "ftm_report_info requires from_opaque — layout differs from ESP-IDF struct");
// clang-format on
...
wifi::connected_info wifi::connected_info::from_opaque(const void* event_data) {
...
wifi::disconnected_info wifi::disconnected_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_sta_disconnected_t*>(event_data);
    disconnected_info result;
    std::memcpy(result.bssid.data(), info->bssid, 6);
    result.ssid = std::string(reinterpret_cast<const char*>(info->ssid), info->ssid_len);
...
wifi::got_ip_info wifi::got_ip_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const ip_event_got_ip_t*>(event_data);
    got_ip_info result;
    result.ip = info->ip_info.ip.addr;
    result.netmask = info->ip_info.netmask.addr;
    result.gateway = info->ip_info.gw.addr;
    result.changed = info->ip_changed;
    return result;
}

wifi::scan_done_info wifi::scan_done_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_sta_scan_done_t*>(event_data);
    return {info->status, info->number, info->scan_id};
}

wifi::authmode_change_info wifi::authmode_change_info::from_opaque(const void* event_data) {
...
wifi::ap_sta_connected_info wifi::ap_sta_connected_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_ap_staconnected_t*>(event_data);
    ap_sta_connected_info result;
    std::memcpy(result.mac.data(), info->mac, 6);
    result.aid = info->aid;
    return result;
}

wifi::ap_sta_disconnected_info wifi::ap_sta_disconnected_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_ap_stadisconnected_t*>(event_data);
    ap_sta_disconnected_info result;
    std::memcpy(result.mac.data(), info->mac, 6);
    result.aid = info->aid;
...
wifi::ap_probe_req_info wifi::ap_probe_req_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_ap_probe_req_rx_t*>(event_data);
    ap_probe_req_info result;
    result.rssi = info->rssi;
    std::memcpy(result.mac.data(), info->mac, 6);
    return result;
}

wifi::bss_rssi_low_info wifi::bss_rssi_low_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_bss_rssi_low_t*>(event_data);
    return {info->rssi};
}

wifi::home_channel_change_info wifi::home_channel_change_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_home_channel_change_t*>(event_data);
    return {
        info->old_chan,
        static_cast<second_channel>(info->old_snd),
        info->new_chan,
        static_cast<second_channel>(info->new_snd)
    };
}

wifi::ftm_report_info wifi::ftm_report_info::from_opaque(const void* event_data) {
    auto* info = static_cast<const wifi_event_ftm_report_t*>(event_data);
    ftm_report_info result;
...
std::string to_string(wifi::mode m) {
    switch (m) {
    case wifi::mode::null:
        return "null";
    case wifi::mode::sta:
