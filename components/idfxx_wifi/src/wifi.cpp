...
static sta_config from_idf_sta_config(const wifi_sta_config_t& s) {
    ...
    cfg.sae_pwe_h2e = flags<sae_pwe_method>(static_cast<int>(s.sae_pwe_h2e));
...
static ap_config from_idf_ap_config(const wifi_ap_config_t& a) {
    ...
    cfg.sae_pwe_h2e = flags<sae_pwe_method>(static_cast<int>(a.sae_pwe_h2e));
    return cfg;
}
...
static wifi_ap_config_t to_idf_ap_config(const ap_config& cfg) {
    ...
    a.sae_pwe_h2e = static_cast<wifi_sae_pwe_method_t>(cfg.sae_pwe_h2e.bits);
    return a;
}
...
result<flags<role>> try_get_roles() {
    ...
    if (err != ESP_OK) {
        ESP_LOGD(TAG, "Failed to get WiFi roles: %s", esp_err_to_name(err));
        return wifi_error(err);
    }
    return flags<role>(static_cast<int>(m));
}

// =============================================================================
// Role to wifi_interface_t mapping
// =============================================================================

static wifi_interface_t to_wifi_if(role r) {
    switch (r) {
    case role::sta:
        return WIFI_IF_STA;
    case role::ap:
...
result<void> try_set_sta_config(const sta_config& cfg) {
    ...
    wifi_cfg.sta.sae_pwe_h2e = static_cast<wifi_sae_pwe_method_t>(cfg.sae_pwe_h2e.bits);
...
result<enum bandwidth> try_get_bandwidth(enum role iface) {
    wifi_bandwidth_t bw;
    auto err = esp_wifi_get_bandwidth(to_wifi_if(iface), &bw);
    if (err != ESP_OK) {
...
result<bandwidths_config> try_get_bandwidths(enum role iface) {
    wifi_bandwidths_t idf_bw{};
    auto err = esp_wifi_get_bandwidths(to_wifi_if(iface), &idf_bw);
    if (err != ESP_OK) {
...
result<mac_address> try_get_mac(enum role iface) {
    mac_address mac;
    auto err = esp_wifi_get_mac(to_wifi_if(iface), mac.data());
    if (err != ESP_OK) {
...
result<channel_info> try_get_channel() {
    ...
    return channel_info{
        primary,
        second == WIFI_SECOND_CHAN_NONE ? std::nullopt : std::optional(static_cast<second_channel>(second)),
    };
}
...
result<flags<protocol>> try_get_protocol(enum role iface) {
    uint8_t bits = 0;
    auto err = esp_wifi_get_protocol(to_wifi_if(iface), &bits);
    if (err != ESP_OK) {
...
result<protocols_config> try_get_protocols(enum role iface) {
    wifi_protocols_t idf_protos{};
    auto err = esp_wifi_get_protocols(to_wifi_if(iface), &idf_protos);
    if (err != ESP_OK) {
...
int64_t get_tsf_time(enum role iface) {
    return esp_wifi_get_tsf_time(to_wifi_if(iface));
}
