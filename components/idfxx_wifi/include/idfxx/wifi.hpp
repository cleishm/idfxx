...
/**
 * @headerfile <idfxx/wifi>
 * @brief WiFi role (station and/or access point).
 *
 * Combined via `flags<role>` to specify which roles are active
...
/**
 * @headerfile <idfxx/wifi>
 * @brief WiFi secondary channel position relative to the primary channel.
 */
...
/** @cond INTERNAL */
template<>
inline constexpr bool idfxx::enable_flags_operators<idfxx::wifi::role> = true;
template<>
...
template<>
inline constexpr bool idfxx::enable_flags_operators<idfxx::wifi::sae_pwe_method> = true;
template<>
...
struct home_channel_change_event_data {
    ...
    std::optional<enum second_channel> old_second; /*!< Previous secondary channel position. */
    uint8_t new_primary;                           /*!< New primary channel. */
    std::optional<enum second_channel> new_second; /*!< New secondary channel position. */
...
inline constexpr idfxx::event<event_id, connected_event_data> sta_connected{event_id::sta_connected};
/** @brief Station disconnected event with disconnection details. */
inline constexpr idfxx::event<event_id, disconnected_event_data> sta_disconnected{event_id::sta_disconnected};
/** @brief Scan completed event with scan results summary. */
inline constexpr idfxx::event<event_id, scan_done_event_data> scan_done{event_id::scan_done};
/** @brief Station authentication mode changed event. */
inline constexpr idfxx::event<event_id, authmode_change_event_data> sta_authmode_change{event_id::sta_authmode_change};
/** @brief Station connected to soft-AP event. */
inline constexpr idfxx::event<event_id, ap_sta_connected_event_data> ap_sta_connected{event_id::ap_sta_connected};
/** @brief Station disconnected from soft-AP event. */
inline constexpr idfxx::event<event_id, ap_sta_disconnected_event_data> ap_sta_disconnected{
    event_id::ap_sta_disconnected
};
/** @brief Probe request received by soft-AP event. */
inline constexpr idfxx::event<event_id, ap_probe_req_event_data> ap_probe_req_received{event_id::ap_probe_req_received};
/** @brief BSS RSSI dropped below threshold event. */
inline constexpr idfxx::event<event_id, bss_rssi_low_event_data> sta_bss_rssi_low{event_id::sta_bss_rssi_low};
/** @brief Home channel changed event. */
inline constexpr idfxx::event<event_id, home_channel_change_event_data> home_channel_change{
    event_id::home_channel_change
};
/** @brief FTM report received event. */
inline constexpr idfxx::event<event_id, ftm_report_event_data> ftm_report{event_id::ftm_report};
...
// =============================================================================
// Free function API — Roles
// =============================================================================

#ifdef CONFIG_COMPILER_CXX_EXCEPTIONS
/**
 * @brief Sets the active WiFi roles.
 *
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] flags<role> get_roles();
#endif
 ...
 * @brief Gets the currently active WiFi roles.
 *
 * @return The active roles, or an error.
 */
[[nodiscard]] result<flags<role>> try_get_roles();
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] enum bandwidth get_bandwidth(enum role iface);
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] bandwidths_config get_bandwidths(enum role iface);
#endif
 ...
 */
[[nodiscard]] result<enum bandwidth> try_get_bandwidth(enum role iface);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_set_bandwidths(enum role iface, const bandwidths_config& bw);
 ...
 */
[[nodiscard]] result<bandwidths_config> try_get_bandwidths(enum role iface);
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] mac_address get_mac(enum role iface);
#endif
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_set_mac(enum role iface, mac_address mac);
 ...
 */
[[nodiscard]] result<mac_address> try_get_mac(enum role iface);
 ...
 * @param second The secondary channel position, or nullopt for HT20 (no secondary channel).
 *
 * @note Only available when CONFIG_COMPILER_CXX_EXCEPTIONS is enabled.
 * @throws std::system_error on failure.
 */
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_set_channel(uint8_t primary, std::optional<enum second_channel> second = std::nullopt);
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] flags<protocol> get_protocol(enum role iface);
 ...
 * @throws std::system_error on failure.
 */
[[nodiscard]] protocols_config get_protocols(enum role iface);
#endif
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_set_protocol(enum role iface, flags<protocol> protos);
 ...
 */
[[nodiscard]] result<flags<protocol>> try_get_protocol(enum role iface);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_set_protocols(enum role iface, const protocols_config& cfg);
 ...
 */
[[nodiscard]] result<protocols_config> try_get_protocols(enum role iface);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_tx_80211(enum role iface, std::span<const uint8_t> buffer, bool en_sys_seq);
 ...
 */
[[nodiscard]] int64_t get_tsf_time(enum role iface);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_config_11b_rate(enum role iface, bool disable);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_config_80211_tx_rate(enum role iface, enum phy_rate rate);
 ...
 * @return Success, or an error.
 */
[[nodiscard]] result<void> try_disable_pmf_config(enum role iface);
...
// Roles
inline void set_roles(flags<role> roles) {
    unwrap(try_set_roles(roles));
}
inline flags<role> get_roles() {
    return unwrap(try_get_roles());
}
...
// Bandwidth
inline void set_bandwidth(enum role iface, enum bandwidth bw) {
    unwrap(try_set_bandwidth(iface, bw));
}
inline enum bandwidth get_bandwidth(enum role iface) {
    return unwrap(try_get_bandwidth(iface));
}
inline void set_bandwidths(enum role iface, const bandwidths_config& bw) {
    unwrap(try_set_bandwidths(iface, bw));
}
inline bandwidths_config get_bandwidths(enum role iface) {
    return unwrap(try_get_bandwidths(iface));
}

// MAC
inline void set_mac(enum role iface, mac_address mac) {
    unwrap(try_set_mac(iface, mac));
}
inline mac_address get_mac(enum role iface) {
...
// Channel
inline void set_channel(uint8_t primary, std::optional<enum second_channel> second) {
...
// Protocol
inline void set_protocol(enum role iface, flags<protocol> protos) {
    unwrap(try_set_protocol(iface, protos));
}
inline flags<protocol> get_protocol(enum role iface) {
    return unwrap(try_get_protocol(iface));
}
inline void set_protocols(enum role iface, const protocols_config& cfg) {
    unwrap(try_set_protocols(iface, cfg));
}
inline protocols_config get_protocols(enum role iface) {
...
inline void config_11b_rate(enum role iface, bool disable) {
    unwrap(try_config_11b_rate(iface, disable));
}
inline void config_80211_tx_rate(enum role iface, enum phy_rate rate) {
    unwrap(try_config_80211_tx_rate(iface, rate));
}
inline void disable_pmf_config(enum role iface) {
...
// =============================================================================
// Default netif creation helpers
// =============================================================================

#ifdef CONFIG_COMPILER_CXX_EXCEPTIONS
/**
 * @brief Creates a default WiFi station network interface.
 *
 * Creates and returns an RAII netif configured as a WiFi station with
 * default settings. Must be called after netif::init().
 *
 ...
 *
 * Creates and returns an RAII netif configured as a WiFi access point with
 * default settings. Must be called after netif::init().
 *
 * @return The network interface.
 *
...
/**
 * @headerfile <idfxx/wifi>
 * @brief Returns a string representation of active WiFi roles.
 *
 * @param roles The active roles.
 * @return "none", "sta", "ap", "sta|ap", or "unknown(N)" for unrecognized values.
 */
[[nodiscard]] std::string to_string(flags<wifi::role> roles);
...
template<>
struct formatter<idfxx::flags<idfxx::wifi::role>> {
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext>
    auto format(idfxx::flags<idfxx::wifi::role> roles, FormatContext& ctx) const {
        auto s = idfxx::to_string(roles);
