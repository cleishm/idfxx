...
TEST_CASE("master_bus scan_devices returns vector", "[idfxx][i2c][master_bus][hw]") {
...
TEST_CASE("master_bus try_probe with invalid address returns error", "[idfxx][i2c][master_bus][hw]") {
    auto bus_result = master_bus::make(port::i2c0, idfxx::gpio_21, idfxx::gpio_26, 100_kHz);
    if (!bus_result.has_value()) {
