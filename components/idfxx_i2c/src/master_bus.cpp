...
static result<i2c_master_bus_handle_t> make_bus(enum port port, gpio sda, gpio scl, freq::hertz frequency) {
    ...
    if (auto err = i2c_new_master_bus(&bus_config, &handle); err != ESP_OK) {
        ESP_LOGD(
            TAG,
            "Failed to create I2C master bus on port %d (SDA: GPIO%d, SCL: GPIO%d, Frequency: %u Hz): %s",
        ...
        case ESP_ERR_NOT_FOUND:
            return std::unexpected(errc::not_found);
        case ESP_ERR_NO_MEM:
            return std::unexpected(errc::no_mem);
        default:
            return std::unexpected(errc::invalid_arg);
        }
    }

    ESP_LOGD(
        TAG,
        "I2C master bus created on port %d (SDA: GPIO%d, SCL: GPIO%d, Frequency: %u Hz)",
        std::to_underlying(port),
        sda.num(),
        scl.num(),
        frequency.count()
...
std::vector<uint8_t> master_bus::_scan_devices(std::chrono::milliseconds timeout) const {
    ...
    if (devices.empty()) {
        ...
        ESP_LOGD(TAG, "  Found %d device(s) on I2C bus %d", devices.size(), std::to_underlying(_port));
    }
    return devices;
}
...
result<void> master_bus::_probe(uint8_t address, std::chrono::milliseconds timeout) const {
    ...
    if (err == ESP_ERR_TIMEOUT) {
        ...
        return std::unexpected(errc::timeout);
    }
    ESP_LOGD(
