...
C++ wrapper for ESP-IDF I2C master driver.

📚 **[Full API Documentation](https://cleishm.github.io/idfxx/group__idfxx__i2c.html)**

## Features

- RAII-based I2C bus and device management
...
try {
    ...
    ESP_LOGI("I2C", "Register value: 0x%02X", data[0]);

} catch (const std::system_error& e) {
    ESP_LOGE("I2C", "I2C error: %s", e.what());
}
```
...
if (!bus_result) {
    auto message = bus_result.error().message();
    ESP_LOGE("I2C", "Failed to create bus: %s", message.c_str());
    return;
}
...
if (!dev_result) {
    auto message = dev_result.error().message();
    ESP_LOGE("I2C", "Failed to create device: %s", message.c_str());
    return;
}
...
if (data_result) {
    ESP_LOGI("I2C", "Register value: 0x%02X", (*data_result)[0]);
} else {
    auto message = data_result.error().message();
    ESP_LOGE("I2C", "Read failed: %s", message.c_str());
}
```
...
ESP_LOGI("I2C", "Found %d device(s):", devices.size());
for (uint8_t addr : devices) {
    ESP_LOGI("I2C", "  - 0x%02X", addr);
}
```
...
ESP_LOGI("I2C", "Received %d bytes", rx_data.size());
```
