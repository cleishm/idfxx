...
C++ wrapper for ESP-IDF LCD panel I/O interface, providing the communication layer for SPI-based LCD panels and touch controllers.

📚 **[Full API Documentation](https://cleishm.github.io/idfxx/group__idfxx__lcd.html)**

## Features

- RAII-based panel I/O lifecycle management
...
if (!spi_bus_result) {
    auto message = spi_bus_result.error().message();
    ESP_LOGE("LCD", "Failed to create SPI bus: %s", message.c_str());
    return;
}
...
if (!panel_io_result) {
    auto message = panel_io_result.error().message();
    ESP_LOGE("LCD", "Failed to create panel I/O: %s", message.c_str());
    return;
}
