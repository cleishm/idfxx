...
// Unit tests for idfxx gpio wrapper
...
TEST_CASE("gpio reset on valid pin succeeds", "[idfxx][gpio][hw]") {
    auto result = gpio::make(0);
    TEST_ASSERT_TRUE(result.has_value());
...
TEST_CASE("gpio set_direction on valid pin succeeds", "[idfxx][gpio][hw]") {
...
TEST_CASE("gpio set_level and get_level", "[idfxx][gpio][hw]") {
    gpio g = gpio::make(0).value();
...
TEST_CASE("gpio pull mode on valid pin", "[idfxx][gpio][hw]") {
    gpio g = gpio::make(0).value();
...
TEST_CASE("gpio interrupt configuration", "[idfxx][gpio][hw]") {
    gpio g = gpio::make(0).value();
