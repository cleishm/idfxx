...
C++ wrapper for ESP-IDF GPIO driver.

📚 **[Full API Documentation](https://cleishm.github.io/idfxx/group__idfxx__gpio.html)**

## Features

- **Type-safe GPIO pin wrapper** with compile-time validation
...
try {
    ...
    ESP_LOGI("GPIO", "Button pressed: %d", pressed);

} catch (const std::system_error& e) {
    ESP_LOGE("GPIO", "Error: %s", e.what());
}
```
...
if (auto result = led.try_set_direction(idfxx::gpio::mode::output); !result) {
    ...
    ESP_LOGE("GPIO", "Failed to set direction: %s", message.c_str());
    return;
}
...
if (auto result = led.try_set_level(true); !result) {
    auto message = result.error().message();
    ESP_LOGE("GPIO", "Failed to set level: %s", message.c_str());
    return;
}
...
if (auto result = button.try_set_pull_mode(idfxx::gpio::pull_mode::pullup); !result) {
    auto message = result.error().message();
    ESP_LOGE("GPIO", "Failed to set pull mode: %s", message.c_str());
    return;
}
...
if (!result) {
    auto message = result.error().message();
    ESP_LOGE("GPIO", "Failed to configure GPIOs: %s", message.c_str());
    return;
}
```
