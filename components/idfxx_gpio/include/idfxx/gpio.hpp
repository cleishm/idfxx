 ...
 * @file gpio.hpp
 * @brief GPIO pin wrapper class.
 *
 * @defgroup idfxx_gpio GPIO Component
 * @brief C++ wrapper for ESP-IDF GPIO driver.
 *
 * Provides type-safe GPIO pin management with compile-time validation,
 * dual error handling (exceptions and result types), and enhanced ISR
 * handler management supporting multiple handlers per pin.
 *
 ...
 * @brief A GPIO pin.
 *
 * Lightweight, non-owning wrapper around a GPIO pin number. Provides type-safe
...
public:
    ...
    /**
     * @brief RAII wrapper for isr_handle that removes the handler on destruction.
     *
...
public:
    ...
    /**
     * @brief Creates a validated GPIO wrapper.
    ...
    /**
     * @brief Constructs a validated GPIO wrapper.
     * @param num GPIO pin number.
    ...
    result<drive_cap> try_get_drive_capability() const {
        if (!is_connected()) {
            return std::unexpected(errc::invalid_state);
        }
    ...
    static void uninstall_isr_service(void) { gpio_uninstall_isr_service(); }
     ...
     *          memory outside of IRAM. Use the raw function pointer overload instead.
     *
