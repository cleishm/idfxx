...
C++ wrapper for STMPE610 resistive touch controller.
...
if (!spi_bus_result) {
    ...
    ESP_LOGE("TOUCH", "Failed to create SPI bus: %s", message.c_str());
    return;
}
...
if (!touch_panel_io_result) {
    auto message = touch_panel_io_result.error().message();
    ESP_LOGE("TOUCH", "Failed to create panel I/O: %s", message.c_str());
    return;
}
...
if (!touch_result) {
    auto message = touch_result.error().message();
    ESP_LOGE("TOUCH", "Failed to create touch controller: %s", message.c_str());
    return;
}
