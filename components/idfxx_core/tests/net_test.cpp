...
static_assert(sizeof(idfxx::net::ipv4_addr) == sizeof(uint32_t));

// ipv6_addr
static_assert(std::is_default_constructible_v<idfxx::net::ipv6_addr>);

// ipv4_info and ipv6_info are aggregates
static_assert(std::is_aggregate_v<idfxx::net::ipv4_info>);
static_assert(std::is_aggregate_v<idfxx::net::ipv6_info>);

// constexpr ipv4_addr construction
static_assert(idfxx::net::ipv4_addr().is_any());
static_assert(idfxx::net::ipv4_addr().addr() == 0);
static_assert(!idfxx::net::ipv4_addr(192, 168, 1, 1).is_any());
static_assert(idfxx::net::ipv4_addr(192, 168, 1, 1) == idfxx::net::ipv4_addr(192, 168, 1, 1));
static_assert(!(idfxx::net::ipv4_addr(192, 168, 1, 1) == idfxx::net::ipv4_addr(192, 168, 1, 2)));

// constexpr ipv6_addr construction
static_assert(idfxx::net::ipv6_addr().is_any());
static_assert(!idfxx::net::ipv6_addr({1, 0, 0, 0}).is_any());
static_assert(idfxx::net::ipv6_addr().zone() == 0);
static_assert(idfxx::net::ipv6_addr({0, 0, 0, 0}, 3).zone() == 3);
static_assert(idfxx::net::ipv6_addr({1, 2, 3, 4}) == idfxx::net::ipv6_addr({1, 2, 3, 4}));
static_assert(!(idfxx::net::ipv6_addr({1, 2, 3, 4}) == idfxx::net::ipv6_addr({1, 2, 3, 5})));
// Zone affects equality
static_assert(!(idfxx::net::ipv6_addr({1, 0, 0, 0}, 1) == idfxx::net::ipv6_addr({1, 0, 0, 0}, 2)));

// =============================================================================
// Runtime tests — ipv4_addr
// =============================================================================

TEST_CASE("ipv4_addr default is zero", "[idfxx][net]") {
    idfxx::net::ipv4_addr addr;
    TEST_ASSERT_EQUAL_UINT32(0, addr.addr());
    TEST_ASSERT_TRUE(addr.is_any());
}

TEST_CASE("ipv4_addr from octets", "[idfxx][net]") {
...
TEST_CASE("ipv4_addr from raw uint32", "[idfxx][net]") {
    // 10.0.0.1 in network byte order (little-endian): 0x0100000a
    idfxx::net::ipv4_addr addr(0x0100000a);
    auto s = idfxx::to_string(addr);
    TEST_ASSERT_EQUAL_STRING("10.0.0.1", s.c_str());
}

TEST_CASE("ipv4_addr equality", "[idfxx][net]") {
...
TEST_CASE("ipv4_addr to_string edge cases", "[idfxx][net]") {
    auto s = idfxx::to_string(idfxx::net::ipv4_addr());
    TEST_ASSERT_EQUAL_STRING("0.0.0.0", s.c_str());
    s = idfxx::to_string(idfxx::net::ipv4_addr(255, 255, 255, 255));
    TEST_ASSERT_EQUAL_STRING("255.255.255.255", s.c_str());
    s = idfxx::to_string(idfxx::net::ipv4_addr(1, 0, 0, 0));
    TEST_ASSERT_EQUAL_STRING("1.0.0.0", s.c_str());
    s = idfxx::to_string(idfxx::net::ipv4_addr(0, 0, 0, 1));
    TEST_ASSERT_EQUAL_STRING("0.0.0.1", s.c_str());
}

// =============================================================================
// Runtime tests — ipv4_info
// =============================================================================

TEST_CASE("ipv4_info to_string", "[idfxx][net]") {
...
TEST_CASE("ipv4_info equality", "[idfxx][net]") {
    idfxx::net::ipv4_info a{
        .ip = idfxx::net::ipv4_addr(10, 0, 0, 1),
        .netmask = idfxx::net::ipv4_addr(255, 0, 0, 0),
        .gateway = idfxx::net::ipv4_addr(10, 0, 0, 254),
    };
    idfxx::net::ipv4_info b = a;
    TEST_ASSERT_TRUE(a == b);
    b.gateway = idfxx::net::ipv4_addr(10, 0, 0, 253);
...
TEST_CASE("ipv6_addr default is zero", "[idfxx][net]") {
    idfxx::net::ipv6_addr addr;
    TEST_ASSERT_TRUE(addr.is_any());
...
TEST_CASE("ipv6_addr with zone", "[idfxx][net]") {
    idfxx::net::ipv6_addr addr({0, 0, 0, 1}, 5);
    TEST_ASSERT_EQUAL_UINT8(5, addr.zone());
    TEST_ASSERT_FALSE(addr.is_any());
}

TEST_CASE("ipv6_addr equality includes zone", "[idfxx][net]") {
    idfxx::net::ipv6_addr a({1, 2, 3, 4}, 0);
    idfxx::net::ipv6_addr b({1, 2, 3, 4}, 0);
    idfxx::net::ipv6_addr c({1, 2, 3, 4}, 1);
    TEST_ASSERT_TRUE(a == b);
    TEST_ASSERT_FALSE(a == c);
}
...
// =============================================================================
// Runtime tests — ipv4_addr::parse
// =============================================================================

TEST_CASE("ip4 parse valid addresses", "[idfxx][net]") {
    auto a = idfxx::net::ipv4_addr::parse("192.168.1.1");
    TEST_ASSERT_TRUE(a.has_value());
    TEST_ASSERT_TRUE(*a == idfxx::net::ipv4_addr(192, 168, 1, 1));

    auto b = idfxx::net::ipv4_addr::parse("0.0.0.0");
    TEST_ASSERT_TRUE(b.has_value());
    TEST_ASSERT_TRUE(b->is_any());

    auto c = idfxx::net::ipv4_addr::parse("255.255.255.255");
    TEST_ASSERT_TRUE(c.has_value());
    TEST_ASSERT_TRUE(*c == idfxx::net::ipv4_addr(255, 255, 255, 255));

    auto d = idfxx::net::ipv4_addr::parse("10.0.0.1");
    TEST_ASSERT_TRUE(d.has_value());
    TEST_ASSERT_TRUE(*d == idfxx::net::ipv4_addr(10, 0, 0, 1));
}

TEST_CASE("ip4 parse roundtrip", "[idfxx][net]") {
    auto original = idfxx::net::ipv4_addr(172, 16, 254, 3);
    auto s = idfxx::to_string(original);
    auto parsed = idfxx::net::ipv4_addr::parse(s);
    TEST_ASSERT_TRUE(parsed.has_value());
    TEST_ASSERT_TRUE(*parsed == original);
}

TEST_CASE("ip4 parse rejects empty", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("").has_value());
}

TEST_CASE("ip4 parse rejects too few octets", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1").has_value());
}

TEST_CASE("ip4 parse rejects too many octets", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.4.5").has_value());
}

TEST_CASE("ip4 parse rejects octet > 255", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("256.0.0.0").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("0.0.0.999").has_value());
}

TEST_CASE("ip4 parse rejects leading zeros", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("01.2.3.4").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.02.3.4").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.04").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("00.0.0.0").has_value());
}

TEST_CASE("ip4 parse rejects trailing chars", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.4 ").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.4x").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.4/24").has_value());
}

TEST_CASE("ip4 parse rejects non-numeric", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("a.b.c.d").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv4_addr::parse("1.2.3.x").has_value());
}
...
// =============================================================================
// Runtime tests — ipv6_addr::parse
// =============================================================================

TEST_CASE("ip6 parse loopback", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("::1");
...
TEST_CASE("ip6 parse all zeros", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("::");
    TEST_ASSERT_TRUE(addr.has_value());
    TEST_ASSERT_TRUE(addr->is_any());
}

TEST_CASE("ip6 parse full address", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("2001:db8:1:2:3:4:5:6");
    TEST_ASSERT_TRUE(addr.has_value());
...
TEST_CASE("ip6 parse with leading compression", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("::1:2:3:4:5:6");
    TEST_ASSERT_TRUE(addr.has_value());
...
TEST_CASE("ip6 parse with trailing compression", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("2001:db8::");
    TEST_ASSERT_TRUE(addr.has_value());
...
TEST_CASE("ip6 parse with middle compression", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("2001:db8::1");
    TEST_ASSERT_TRUE(addr.has_value());
...
TEST_CASE("ip6 parse uppercase hex", "[idfxx][net]") {
    auto addr = idfxx::net::ipv6_addr::parse("ABCD:EF01::1");
    TEST_ASSERT_TRUE(addr.has_value());
...
TEST_CASE("ip6 parse roundtrip", "[idfxx][net]") {
    ...
    auto s = idfxx::to_string(original);
    auto parsed = idfxx::net::ipv6_addr::parse(s);
    TEST_ASSERT_TRUE(parsed.has_value());
    TEST_ASSERT_TRUE(*parsed == original);
}
...
TEST_CASE("ip6 parse rejects empty", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("").has_value());
}

TEST_CASE("ip6 parse rejects triple colon", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse(":::").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("1:::2").has_value());
}

TEST_CASE("ip6 parse rejects double compression", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("1::2::3").has_value());
}

TEST_CASE("ip6 parse rejects too many groups", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("1:2:3:4:5:6:7:8:9").has_value());
}

TEST_CASE("ip6 parse rejects too few groups without compression", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("1:2:3:4:5:6:7").has_value());
}

TEST_CASE("ip6 parse rejects group > 4 hex digits", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("12345::1").has_value());
}

TEST_CASE("ip6 parse rejects non-hex characters", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("gggg::1").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("1:2:3:4:5:6:7:xyz").has_value());
}
...
TEST_CASE("ip6 parse rejects empty zone", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("fe80::1%").has_value());
}

TEST_CASE("ip6 parse rejects non-numeric zone", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("fe80::1%eth0").has_value());
}

TEST_CASE("ip6 parse rejects zone > 255", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("fe80::1%256").has_value());
}

TEST_CASE("ip6 parse rejects trailing chars", "[idfxx][net]") {
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("::1 ").has_value());
    TEST_ASSERT_FALSE(idfxx::net::ipv6_addr::parse("::1/64").has_value());
}

TEST_CASE("ip6 parse compression expanding single group", "[idfxx][net]") {
    // 1:2:3:4:5:6:7:: — :: represents exactly one zero group
    auto addr = idfxx::net::ipv6_addr::parse("1:2:3:4:5:6:7::");
    TEST_ASSERT_TRUE(addr.has_value());
