 ...
 * @endcode
 */
class datagram_socket : public detail::connectionless_socket_base {
public:
    /**
     ...
     * @throws std::system_error on failure.
     */
    [[nodiscard]] explicit datagram_socket(enum family fam);
#endif
     ...
     * @return The new socket, or an error.
     */
    [[nodiscard]] static result<datagram_socket> make(enum family fam);
     ...
     * @throws std::system_error on failure.
     */
    [[nodiscard]] explicit datagram_socket(const config& cfg);
#endif
     ...
     * @retval errc::invalid_state the socket has been moved from
     */
    result<void> try_set_multicast_interface_v6(uint8_t if_index);
#endif

    /// @cond INTERNAL
    /**
     * @brief Constructs a datagram_socket from an existing descriptor.
     */
    datagram_socket(int fd, enum family fam) noexcept
