...
static_assert(std::to_underlying(idfxx::net::address_family::ipv4) == AF_INET);
#ifdef CONFIG_LWIP_IPV6
static_assert(std::to_underlying(idfxx::net::address_family::ipv6) == AF_INET6);
#endif

namespace idfxx::net::detail {

int family_to_af(std::optional<address_family> f) noexcept {
    if (!f) {
        return AF_UNSPEC;
    }
    switch (*f) {
