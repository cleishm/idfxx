...
// event_group<test_event> is not default constructible without exceptions
static_assert(!std::is_default_constructible_v<event_group<test_event>>);
#endif
...
TEST_CASE("event_group::make succeeds", "[idfxx][event_group]") {
    auto result = event_group<test_event>::make();
...
TEST_CASE("event_group blocking wait any - other task sets bits", "[idfxx][event_group]") {
    auto eg_result = event_group<test_event>::make();
    TEST_ASSERT_TRUE(eg_result.has_value());
    auto& eg = *eg_result;

    std::atomic<bool> received{false};

    // Waiter task: blocks waiting for event_a
    auto t_result = task::make({.name = "eg_wait"}, [&eg, &received](task::self&) {
...
TEST_CASE("event_group blocking wait all - two tasks set different bits", "[idfxx][event_group]") {
    ...
    auto waiter = task::make({.name = "eg_all"}, [&eg, &received](task::self&) {
        auto r = eg->try_wait(test_event::event_a | test_event::event_b, wait_mode::all, 500ms);
        if (r.has_value()) {
            received.store(true);
        }
    });
    TEST_ASSERT_TRUE(waiter.has_value());

    // Task 1 sets event_a
    auto setter1 = task::make({.name = "eg_set1"}, [&eg](task::self&) {
        idfxx::delay(50ms);
        eg->set(test_event::event_a);
    });
    TEST_ASSERT_TRUE(setter1.has_value());

    // Task 2 sets event_b
    auto setter2 = task::make({.name = "eg_set2"}, [&eg](task::self&) {
        idfxx::delay(100ms);
        eg->set(test_event::event_b);
    });
    TEST_ASSERT_TRUE(setter2.has_value());
...
TEST_CASE("event_group sync - two tasks rendezvous", "[idfxx][event_group]") {
    ...
    auto t1 = task::make({.name = "eg_sync1"}, [&eg, &task1_synced, wait_bits](task::self&) {
        auto r = eg->try_sync(test_event::event_a, wait_bits, 500ms);
        if (r.has_value()) {
            task1_synced.store(true);
        }
    });
    TEST_ASSERT_TRUE(t1.has_value());

    // Task 2: sets event_b, waits for both
    auto t2 = task::make({.name = "eg_sync2"}, [&eg, &task2_synced, wait_bits](task::self&) {
        auto r = eg->try_sync(test_event::event_b, wait_bits, 500ms);
        if (r.has_value()) {
            task2_synced.store(true);
        }
    });
    TEST_ASSERT_TRUE(t2.has_value());
